...
import com.rama.puma.widgets.WdColorPicker
import com.rama.puma.widgets.WdRange
...
class SettingsAppearanceController(private val activity: SettingsActivity) {
    ...
    private fun setupTheme() {
        ...
        when (prefs.getTheme()) {
            ...
            PrefsManager.Theme.MAKO -> group.check(R.id.theme_puma)
            ...
            else -> group.check(R.id.theme_puma)
        }

        group.setOnCheckedChangeListener { _, id ->
            val theme = when (id) {
                R.id.theme_rama -> PrefsManager.Theme.RAMA
                R.id.theme_puma -> PrefsManager.Theme.MAKO
